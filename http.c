...
struct http_response *http_response_init(struct worker *w, int code, const char *msg) {
	...
	http_response_set_header(r, "Server", "Pbiws");

	return r;
}


void http_response_set_header(struct http_response *r, const char *k, const char *v) {
...
void http_response_set_body(struct http_response *r, const char *body, size_t body_len) {
	char *buffer;
	buffer = strdup(body);
	free(r->body);
	r->body = buffer;
...
void http_response_add_body(struct http_response *r, const char *body, size_t body_len) {
	r->body = realloc(r->body, r->body_len + body_len + 1);
	memcpy(r->body + r->body_len, body, body_len);
	r->body_len += body_len;
	r->body[r->body_len] = 0;
}
...
static void http_can_write(int fd, short event, void *p) {
	int ret;
...
void http_schedule_write(int fd, struct http_response *r) {
	if(r->w) { /* async */
		struct event *ev = event_new(r->w->base, fd, EV_WRITE, http_can_write, (void*) r);
		event_add(ev, NULL);
	} else { /* blocking */
...
static void http_response_set_connection_header(struct http_client *c, struct http_response *r) {
